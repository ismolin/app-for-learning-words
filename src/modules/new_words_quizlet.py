...
class NewWordsQuizlet:
    ...
    async def create_quantity_of_words_from_category_list(self):
        ...
        await db.execute_void(query=f"""TRUNCATE TABLE {self.user_name}_days_words_list""")
        ...
        total_quantity_of_words_query = await db.execute_one(query=f"""SELECT total_quantity_of_words                           
        ...
        quantity_of_categories_query = await db.execute_one(query=f"""SELECT COUNT(categories) 
                                                               FROM {self.user_name}_info""")
    ...
    async def create_new_words_list(self, categories_list: list, quantity_of_words_from_category_list: list):
        ...
        for categories, local_count_of_words in zip(categories_list, quantity_of_words_from_category_list):
            words = await db.execute_one(f"""SELECT {categories}.words_eng, {categories}.words_rus
                                        FROM {categories}
                                        WHERE {categories}.words_eng NOT IN
                                        (SELECT words_eng FROM {self.user_name}_words)
                                        ORDER BY RANDOM()
                                        LIMIT {local_count_of_words}""")
            await db.execute_many(f"""INSERT INTO {self.user_name}_days_words_list(words_eng, words_rus) 
                                     VALUES ($1, $2)""", words)

    async def create_categories_list(self):
        """Generating a list with user categories"""

        categories_import = await db.execute_one(query=f"""SELECT categories 
                                                    FROM {self.user_name}_info 
    ...
    async def create_quizlet_card(self):
        ...
        result = await db.execute_one(f'''SELECT words_eng, words_rus 
                                     FROM {self.user_name}_days_words_list 
                                     LIMIT 1''')
        spoiler = f'||{result[0][1]}||'
        return result[0][0] + '\n' + f'{spoiler}'
    ...
    async def generate_new_word(self, categories):
        ...
        new_word = await db.execute_one(query=f'''SELECT words_eng, words_rus FROM {category} WHERE words_eng NOT IN 
                                           (SELECT words_eng from {self.user_name}_days_words_list
                                           UNION 
                                           SELECT words_eng from {self.user_name}_words) 
                                           ORDER BY RANDOM()
                                           LIMIT 1''')
        await db.execute_void(query=f"""INSERT INTO {self.user_name}_days_words_list(words_eng, words_rus) 
    ...
    async def i_know_this_word(self):
        ...
        await db.execute_void(query=f"""DELETE FROM {self.user_name}_days_words_list 
    ...
    async def send_quizlet_card(self, next_word_card: str, start_quizlet: bool):
        ...
        if start_quizlet:
            ...
            if len(await db.execute_one(query=f"""SELECT * FROM {self.user_name}_words 
    ...
    async def this_is_last_word(self):
        ...
        return len(await db.execute_one(query=f"""SELECT * FROM {self.user_name}_days_words_list""")) == 1
