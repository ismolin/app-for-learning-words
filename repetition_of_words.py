...
async def start_repeating_words(message, bot):
    ...
    if len(words) == 0:
        ...
        await db_update_many(f"""INSERT INTO {message.from_user.username}_repetition_list(words_eng, words_rus, words_count) 
                                 VALUES %s""", words)
        await bot.send_message(message.from_user.id, f'Тебе доступно {len(words)} слов для повтора!', reply_markup=GeneralMenuButton)
...
async def repetition_words(message, bot):
    ...
    if message.data == 'Я вспомнил это слово':
        ...
        if len(await db_select(sql = f"""SELECT * FROM {message.from_user.username}_repetition_list WHERE words_eng NOT IN ('{woerds_eng}')""")) == 0:
            print("OK")
            ...
            await bot.send_message(message.from_user.id, f'''На этом пока все, следующие слова для повтора будут доступны\nчерез {time[0]} ч {time[1]} мин!''',
                                   parse_mode='HTML',
